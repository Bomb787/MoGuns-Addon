...
public class BlueHeatModel implements IOverrideModel {
	...
	public void render(float partialTicks, TransformType transformType, ItemStack stack, ItemStack parent, LivingEntity entity, MatrixStack matrixStack, IRenderTypeBuffer buffer, int light, int overlay) {
		...
		RenderUtil.renderModel(SpecialModels.BLUE_HEAT_MAIN.getModel(), stack, matrixStack, buffer, light, overlay);
		
		if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.LIGHT_STOCK.get())
            RenderUtil.renderModel(SpecialModels.LIGHT_BLUE_HEAT_STOCK.getModel(), stack, matrixStack, buffer, light, overlay);
		
		if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.TACTICAL_STOCK.get())
            RenderUtil.renderModel(SpecialModels.TACTICAL_BLUE_HEAT_STOCK.getModel(), stack, matrixStack, buffer, light, overlay);
		
		if(Gun.getAttachment(IAttachment.Type.STOCK, stack).getItem() == ModItems.WEIGHTED_STOCK.get())
			RenderUtil.renderModel(SpecialModels.HEAVY_BLUE_HEAT_STOCK.getModel(), stack, matrixStack, buffer, light, overlay);
		...
		if(entity.equals(Minecraft.getInstance().player)) {
            ...
            RenderUtil.renderModel(SpecialModels.BLUE_HEAT_SLIDE.getModel(), stack, matrixStack, buffer, light, overlay);
            //Always pop
            matrixStack.popPose();
